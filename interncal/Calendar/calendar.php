    ...
    **/
    private function _showDay($cellNumber) {
      ...
      if (($this->currentDay != 0) && ($this->currentDay <= $this->daysInMonth)) {
        ...
        $cellContent = $this->currentDay; //the code responsible for printing the data in cell
        $this->currentDay++;
      } else {
      ...
      $today_mon = date("m");
      $today_yea = date("Y");
      $date=date("Y-m-d", strtotime($this->currentYear."-".$this->currentMonth."-".$cellContent));
      $sql="select * from register where EventDate='$date'";
      $result=$conn->query($sql);
      if($result->num_rows>0)
      {
        $class_day = "calendar_event";
        $class_day .= ($cellContent == $today_day && $this->currentMonth == $today_mon && $this->currentYear == $today_yea ? " calendar_today" : "");
      }
      else
      {
      ...
      return '<li class="' . $class_day . '">' . "<a style=display:block href=index.php?year=$year&month=$month&day=$cellContent>" . $cellContent . ' </a> </li>' . "\r\n";
    }
    /**
    ** create navigation
    **/
